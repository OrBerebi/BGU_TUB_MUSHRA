...
class JackClientHandler(object):
    def __init__(self, sampling_rate, num_ssr_srcs, num_real_src=0):
        """
        Listening Experiment Py: Jack handler
        ...
        self._num_real_src = num_real_src  # these sources are directy routed
                                           # to system output
        # open new jack audio thread
        self._audio_thread = JackAudioThread(sampling_rate, num_ssr_srcs,
                                             num_real_srcs=num_real_src)

        # set playback source

        self._audio_thread.set_source('file_playback',
                                      'src/test_signals/MARA_LE_DRUMS.wav')
    ...
    def select_real_src(self, id):
        for i in range(self._num_real_src):
            self._audio_thread.disconnect_direct(i)

        self._audio_thread.connect_direct(id)

class JackAudioThread(threading.Thread):
    def __init__(self, fs, num_ssr_srcs, num_real_srcs=0, num_buffers=5):
        """
        ...
        self._fs = fs
        self._num_ssr_srcs = num_ssr_srcs
        self._num_real_srcs = num_real_srcs
    ...
    def run(self):
        ...
        # do jack routing
        self.jack_routing(self._num_ssr_srcs, self._num_real_srcs)
    ...
    def jack_routing(self, num_ssr_srcs, num_real_src=0):
        ...
        for idx in range(0, num_real_src):  # channels that are directly routed to output
            try:
                self._jack_client.connect(f'{self._id}:output_1', 'system:playback_{:01d}'.format(idx + 3))
            except:
                print('direct routing of src {:01d} failed'.format(idx))

    def disconnect_direct(self, id):
        try:
            self._jack_client.disconnect(f'{self._id}:output_1', 'system:playback_{:01d}'.format(id + 3))
        except:
            print('direct disconnect of src {:01d} failed'.format(id))

    def connect_direct(self, id):
        try:
            self._jack_client.connect(f'{self._id}:output_1', 'system:playback_{:01d}'.format(id + 3))
        except:
            print('direct connect of src {:01d} failed'.format(id))
